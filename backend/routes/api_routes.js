...
// route to add job to a database
...
// to get information about a particular job
...
// to update information of a particular job
...
// recruiter/applicant are shown all their applications
router.get("/applications", jwtAuth, (req, res) => {
   ...
   if (user.type == "recruiter") {
      Applications.findAll({
         where: { rid: user.uid },
         include: [
            { model: JOB, order: ["dateOfPosting", "ASC"] },
            { model: Recruiter },
         ],
      })
         .then((applications) => {
